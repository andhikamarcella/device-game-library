...
export default function DashboardPage() {
  ...
  const { games, addGame, toggleWishlistGame } = useGameStore();
  ...
  const handleAddToWishlist = (game: SearchResult) => {
    ...
    if (existing) {
      if (!existing.wishlist) {
        ...
        setWishlistStatus({ message: `${game.name} dipindahkan ke wishlist kamu.`, tone: "success" });
      } else {
    ...
    setWishlistStatus({ message: `${game.name} berhasil ditambahkan ke wishlist.`, tone: "success" });
  };
          ...
          <div className="flex flex-wrap items-center gap-2 overflow-x-auto rounded-xl bg-slate-100/60 p-2 dark:bg-slate-800/60">
            ...
            {retroPlatforms.map((platform) => (
              <button
                key={platform.id}
                type="button"
                onClick={() => setSelectedPlatform(String(platform.id))}
                  ...
                  <div className="flex flex-1 flex-col justify-between gap-3 p-4">
                    ...
                    <div className="flex items-center justify-between gap-3 pt-1">
                      ...
                      >
                        ...
                        {isWishlisted ? "Sudah di wishlist" : "Tambah ke wishlist"}
                      </button>
                    </div>
                  </div>
                </article>
