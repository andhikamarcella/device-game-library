          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {results.map((game) => (
              <Link
                key={game.id}
                href={`/games/${game.id}`}
                className="group overflow-hidden rounded-2xl border border-slate-200 bg-white/80 text-slate-900 shadow-md shadow-slate-900/10 transition-transform duration-200 hover:-translate-y-1 hover:border-emerald-400 hover:shadow-emerald-500/20 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-100"
              >
