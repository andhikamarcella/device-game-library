    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 md:flex-row md:items-center">
          ...
          <button
            type="submit"
            className="inline-flex items-center justify-center rounded-xl border border-emerald-500/50 bg-emerald-500/20 px-4 py-2 text-sm font-semibold text-emerald-200 transition hover:border-emerald-400 hover:text-white"
          >
        ...
        <div className="mt-4 space-y-4">
          ...
          {!loading && !error && hasSearched && results.length === 0 ? (
            <p className="text-sm text-slate-400">No games matched your search. Try a different title.</p>
          ) : null}
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {results.map((game) => (
              <Link
                key={game.id}
                href={`/games/${game.id}`}
                className="group overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60 transition hover:border-emerald-500/40 hover:bg-slate-900"
              >
                <div className="relative aspect-video w-full overflow-hidden bg-slate-800">
                  ...
                  <div className="absolute right-3 top-3 flex items-center gap-1 rounded-full bg-slate-950/80 px-2 py-1 text-xs text-amber-300">
                ...
                <div className="space-y-2 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">{game.releaseYear ?? "Unknown year"}</p>
                    </div>
                    <ArrowRight className="h-4 w-4 text-emerald-300 opacity-0 transition group-hover:translate-x-1 group-hover:opacity-100" />
                  </div>
                  {game.platforms.length ? (
                    <p className="text-xs text-slate-400">{game.platforms.join(", ")}</p>
                  ) : (
