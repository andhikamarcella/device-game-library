...
import type { Device, Game, Settings, UserGameEntry } from "@/lib/types";
import { cn } from "@/lib/utils";

export default function SettingsPage() {
  const { devices, replaceDevices } = useDeviceStore();
  const { games: legacyGames, replaceGames } = useGameStore();
  const { settings, updateSettings } = useSettingsStore();
  const { theme, setTheme } = useThemePreference();
  const { games: userLibrary, replaceAll: replaceUserLibrary, clearAll: clearUserLibrary } = useUserGameLibrary();
  const [importError, setImportError] = useState<string | null>(null);
  const [importSuccess, setImportSuccess] = useState<string | null>(null);

  const accessibilityOptions: Array<{
    key: keyof Pick<Settings, "reduceMotion" | "highContrast" | "largeText">;
    label: string;
    description: string;
    icon: LucideIcon;
  }> = [
    {
      key: "reduceMotion",
      label: "Reduce motion",
      description: "Minimize interface animations for motion comfort.",
      icon: Gauge,
    },
    {
      key: "highContrast",
      label: "High contrast",
      description: "Boost contrast for improved visibility across the app.",
      icon: Contrast,
    },
    {
      key: "largeText",
      label: "Larger text",
      description: "Increase font sizing for easier reading on every page.",
      icon: Type,
    },
  ];
  ...
  const handleAccessibilityToggle = (
    ...
    updateSettings({ [key]: nextValue } as Partial<Settings>);
  };
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      if (payload.userLibrary) {
        replaceUserLibrary(payload.userLibrary);
      }
      if (payload.settings) {
      ...
      setImportSuccess(
        `Imported ${payload.devices.length} devices, ${payload.games.length} ROM games, and ${payload.userLibrary?.length ?? 0} library entries.`,
      );
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-6">
          ...
          {[
            ...
            return (
              <button
                ...
                className={cn(
                  "rounded-xl border px-4 py-2 text-sm font-medium transition-transform duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-50 dark:focus-visible:ring-offset-slate-900",
                  isActive
                    ...
                    : "border-slate-200 bg-white text-slate-600 hover:-translate-y-0.5 hover:border-emerald-300 hover:text-emerald-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300",
                )}
      ...
      <Card
        title="Accessibility"
        description="Adjust contrast, animation, and text size for a more comfortable experience."
      >
        <div className="grid gap-3 md:grid-cols-3">
          {accessibilityOptions.map((option) => {
            const isActive = Boolean(settings[option.key]);
            ...
            return (
              <button
                ...
                className={cn(
                  "flex h-full flex-col items-start gap-2 rounded-xl border px-4 py-3 text-left text-sm font-semibold shadow-sm transition-transform duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-50 dark:focus-visible:ring-offset-slate-900",
                  isActive
                    ? "border-emerald-500/60 bg-emerald-500/15 text-emerald-700 dark:border-emerald-500/50 dark:bg-emerald-500/20 dark:text-emerald-200"
                    : "border-slate-200 bg-white text-slate-600 hover:-translate-y-0.5 hover:border-emerald-300 hover:text-emerald-700 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300",
                )}
                aria-pressed={isActive}
              >
                ...
                <span className="text-xs font-normal leading-relaxed text-slate-500 dark:text-slate-400">
                  {option.description}
                </span>
              </button>
            );
      ...
      >
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <label
                className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm transition-transform duration-200 hover:-translate-y-0.5 hover:border-emerald-300 hover:text-emerald-700 focus-within:outline-none focus-within:ring-2 focus-within:ring-emerald-500 focus-within:ring-offset-2 focus-within:ring-offset-white dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:text-white dark:focus-within:ring-offset-slate-900"
            >
          ...
          {importSuccess ? (
            <p className="text-sm text-emerald-600 dark:text-emerald-300" role="status" aria-live="polite">
              {importSuccess}
            </p>
          ) : null}
          {importError ? (
            <p className="text-sm text-rose-600 dark:text-rose-300" role="alert" aria-live="assertive">
              {importError}
            </p>
          ) : null}
        </div>
      ...
      <Card title="Data overview" description="Snapshot of what is stored in your browser.">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/10 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/10 dark:border-slate-800 dark:bg-slate-900">
            ...
            <p className="mt-2 text-3xl font-semibold text-slate-900 dark:text-white">{legacyGames.length}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/10 dark:border-slate-800 dark:bg-slate-900">
