        ...
        <div className="space-y-6 p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-2">
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white sm:text-3xl lg:text-4xl">{game.name}</h1>
              <p className="text-sm text-slate-600 dark:text-slate-400">Released: {releaseDateLabel}</p>
            ...
            <div className="flex items-center gap-2 rounded-2xl border border-amber-400/60 bg-amber-100 px-4 py-2 text-amber-700 shadow-sm dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
          ...
          {genres.length ? (
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-600 dark:text-slate-400">Genres</h2>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{genres.join(", ")}</p>
            </div>
          ) : null}

          {developers.length ? (
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-600 dark:text-slate-400">Developers</h2>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{developers.join(", ")}</p>
            </div>
          ) : null}

          {publishers.length ? (
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-widest text-slate-600 dark:text-slate-400">Publishers</h2>
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{publishers.join(", ")}</p>
            </div>
          ) : null}
