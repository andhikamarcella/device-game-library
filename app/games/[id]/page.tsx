...
export default async function GameDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-12 pb-16">
      <div className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-slate-900 shadow-xl shadow-slate-950/40 dark:border-slate-800">
        {heroImage && (
        ...
        <div className="relative grid gap-10 p-8 md:grid-cols-[220px_1fr] md:p-12">
          <div className="flex flex-col items-start gap-4">
            <div className="overflow-hidden rounded-3xl border border-slate-200/60 bg-slate-900/60 shadow-2xl shadow-slate-950/40 dark:border-slate-700">
              {coverImage ? (
                <img src={coverImage} alt="Game cover art" className="h-full w-full object-cover" />
              ) : (
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 rounded-full border border-slate-200/60 bg-slate-900/40 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-200 transition hover:border-emerald-400 hover:text-emerald-300"
            >
              ← Back to search
            </Link>
          </div>
