...
type RawgDetailsResponse = {
  ...
  short_screenshots?: Array<{ id: number; image: string | null; width?: number; height?: number }>;
};
...
export async function GET(_request: Request, { params }: Params) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      background_image: normalizeRawgImageUrl(data.background_image),
      background_image_additional: normalizeRawgImageUrl(data.background_image_additional),
      ...
      short_screenshots:
        data.short_screenshots?.map((shot) => ({
          ...shot,
          image: normalizeRawgImageUrl(shot.image),
        })) ?? [],
    });
  } catch (error) {
