...
export async function GET(req: NextRequest) {
  try {
    ...
    const requestedPage = Number.isFinite(pageCandidate) && pageCandidate > 0 ? pageCandidate : 1;
    const MAX_OFFSET = 5000;
    const maxPage = Math.max(1, Math.floor(MAX_OFFSET / pageSize) + 1);
    ...
    const queryParts: string[] = [`fields ${fieldSelections.join(", ")};`];
    ...
    if (error instanceof IgdbRequestError) {
      console.error("IGDB games search error", {
        status: error.status,
        body: error.body,
        message: error.message,
      });
      const statusCode = error.status && [400, 401, 403, 404].includes(error.status) ? error.status : 502;
