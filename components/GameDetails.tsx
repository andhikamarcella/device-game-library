...
const storeIconMap: Record<string, JSX.Element> = {
  steam: <Store className="h-4 w-4" />,
  "playstation-store": <Gamepad2 className="h-4 w-4" />,
  "xbox-store": <Gamepad2 className="h-4 w-4" />,
  "nintendo": <Gamepad2 className="h-4 w-4" />,
  gog: <ShoppingBag className="h-4 w-4" />,
  epic: <ShoppingBag className="h-4 w-4" />,
  "epic-games-store": <ShoppingBag className="h-4 w-4" />,
};
...
export function GameDetails({
  ...
  const ratingCountLabel = Number.isFinite(game.ratings_count) ? game.ratings_count.toLocaleString() : "0";
  const heroImage = normalizeRawgImageUrl(game.background_image_additional ?? game.background_image ?? null);
  const thumbnailImage = normalizeRawgImageUrl(
    game.background_image ?? game.background_image_additional ?? screenshots[0]?.image ?? null,
  );
              ...
              <div className="space-y-3">
                ...
                {uniquePlatforms.length ? (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Available on</p>
        ...
        <section className="space-y-4 rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
          ...
          <div className="flex flex-wrap gap-3">
            {storeEntries.map((store) => {
              ...
              const icon = store.store.slug && storeIconMap[store.store.slug];
              return (
                <a
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {additionEntries.map((addition) => {
              ...
              return (
                <Link
                  key={addition.id}
                  href={`/games/${addition.slug ?? addition.id}`}
                    ...
                    {additionPlatforms.length ? (
                      <div className="flex items-center gap-2 text-slate-500">
                        {additionPlatforms.map((platform) => (
                          <span key={platform.platform.id}>{getPlatformIcon(platform.platform.slug, platform.platform.name)}</span>
                        ))}
                      </div>
                    ) : null}
                  </div>
        ...
        <section className="space-y-3 rounded-3xl border border-slate-200 bg-white/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {seriesEntries.slice(0, 6).map((entry) => (
              <Link
                key={entry.id}
                href={`/games/${entry.slug ?? entry.id}`}
