...
export function LayoutSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const SidebarContent = (
    <div className="flex h-full flex-col border-r border-slate-200 bg-white text-slate-900 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100">
      <div className="flex items-center justify-between px-6 py-5">
        <div className="flex items-center gap-3">
          <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-emerald-500/30 bg-emerald-500/10 shadow-sm shadow-emerald-500/20">
            <div
              aria-hidden
        ...
        <button
          type="button"
          onClick={onClose}
          className="rounded-lg p-2 text-slate-500 transition hover:bg-slate-200 hover:text-slate-900 focus-visible:ring-2 focus-visible:ring-emerald-500 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-100 lg:hidden"
        ...
        {navigation.map((item, index) => {
          ...
          return (
            <Link
              ...
              onClick={onClose}
              className={cn(
                "flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-50 motion-safe:animate-[menu-item-appear_0.25s_ease-out] motion-safe:transition-transform motion-safe:hover:translate-x-1 dark:focus-visible:ring-offset-slate-900",
                active
      ...
      <div className="border-t border-slate-200 px-6 py-5 text-xs text-slate-500 transition-colors duration-300 dark:border-slate-800 dark:text-slate-400">
