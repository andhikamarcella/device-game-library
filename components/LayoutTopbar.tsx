...
export function LayoutTopbar({ onOpenSidebar }: { onOpenSidebar: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full flex-none items-center gap-3 border-b border-slate-200 bg-white px-4 text-slate-900 transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-100 sm:px-6">
      <button
        type="button"
        ...
        {showSearch ? (
          <div className="relative ml-auto w-full max-w-md">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white py-2 pl-10 pr-4 text-sm text-slate-900 placeholder:text-slate-500 shadow-sm transition-colors focus:border-emerald-500 focus:ring-emerald-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
            />
          </div>
        ) : null}
      </div>
