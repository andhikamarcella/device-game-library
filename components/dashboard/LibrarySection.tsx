  ...
  return (
    <section className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.45em] text-emerald-500">Library</p>
          <h2 className="mt-1 text-2xl font-semibold text-slate-900 dark:text-white">Manage ownership, play status, and notes</h2>
        </div>
        <div className="flex flex-wrap items-center gap-3 text-xs sm:text-sm">
                ...
                <button
                  ...
                  className={`rounded-full border px-3 py-1 transition ${
                    active
                      ...
                      : "border-slate-200 text-slate-500 hover:border-emerald-400 hover:text-emerald-500 dark:border-white/10 dark:text-slate-300"
                  }`}
                >
                  {option.replace(/_/g, " ")}
                </button>
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white/80 px-3 py-1 dark:border-white/10 dark:bg-[#0e1d36]/80"
          >
          ...
          <label className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-16 rounded border border-slate-200 px-2 py-1 text-center text-sm dark:border-white/10 dark:bg-[#0e1d36]/80"
            />
          </label>
          <select
            value={sort}
      ...
      {sorted.length === 0 ? (
        <p className="rounded-2xl border border-slate-200/60 bg-white/80 p-6 text-center text-sm text-slate-600 dark:border-white/10 dark:bg-[#0a1930]/80 dark:text-slate-300">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {sorted.map((entry) => (
            <article
              key={entry.rawgId}
              className="flex flex-col gap-3 rounded-2xl border border-slate-200/60 bg-white/80 p-4 shadow-lg shadow-emerald-500/5 transition hover:-translate-y-1 hover:shadow-emerald-500/20 dark:border-white/10 dark:bg-[#0a1b33]/85"
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between gap-3 text-xs text-slate-500 dark:text-slate-300">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-md border border-rose-500/60 px-3 py-1 text-sm font-semibold text-rose-500 transition hover:border-rose-500 hover:bg-rose-500/15 hover:text-rose-400 dark:text-rose-300"
                  >
              ...
              {entry.notes && (
                <p className="rounded-lg bg-white/70 p-3 text-sm text-slate-600 dark:bg-[#0f213d]/80 dark:text-slate-300">
                  {entry.notes}
                </p>
              )}
            </article>
